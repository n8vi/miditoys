...
describe (midimsg* event)
{
	...
	switch (type) {
		case 0x90:
			assert (event->size == 3);
			printf(" ON: chan %2d vel %3d freq %f", channel, event->buffer[2], exp2(event->buffer[1]/12.0));
			break;
		case 0x80:
			assert (event->size == 3);
			printf("OFF: chan %2d vel %3d freq %f", channel, event->buffer[2], exp2(event->buffer[1]/12.0));
			break;
		case 0xb0:
			assert (event->size == 3);
			printf(" CC: chan %2d ctl %3d  val %3d", channel, event->buffer[1], event->buffer[2]);
			break;
		default:
			break;
	}
...
static void usage (int status) {
	printf ("jsynthosc - JACK Synth Oscillator.\n\n");
	printf ("Usage: jsynthosc [ OPTIONS ] [CLIENT-NAME]\n\n");
	printf ("Options:\n\
...
\n\
If no client name is given it defaults to 'jsynthosc'.\n\
\n\
See also: jackd(1)\n\
\n");
	exit (status);
